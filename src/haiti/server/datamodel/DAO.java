 ...
 * 
 * <BR>
 * To compile: javac -classpath ../../haiti-server.jar:. SmsReader.java <BR>
 * To run: java -classpath ../../haiti-server.jar:. SmsReader <filename>
 * [encoding]
 * 
 */
public class DAO {
	...
	// UNKNOWN(-1), NEW(0), PENDING(1), PROCESSED(2), DECLINED(3), ARCHIVED(4),
	// ALL(5);
	//
	// private int code;
	...
	public static final String DB_AV_NUM = "av_num";

	public static final String ID_ALIAS_1 = "first_id";
	public static final String ID_ALIAS_2 = "second_id";
	// public static final String AttributeManager.PAIRS_SEPARATOR =
	// AttributeManager.;
	...
	/**
	 * Reads messages from an Sqlite database. Details for implementing the
	 * DataBase can be found on the below link
	...
	public void readUnprocessedMsgsFromDb(String dbName) {
		try {
			...
			while (!rs.isAfterLast()) {
				String msg = DB_MESSAGE_ID + AttributeManager.ATTR_VAL_SEPARATOR + rs.getString(DB_MESSAGE_ID)
	...
	public void createNewUser(DataEntryGUI gui, String dbName, String user, String password) {
		try {
			...
			if (rs.isAfterLast()) {
				statement.executeUpdate(AttributeManager.NEW_USER + user + AttributeManager.SINGLE_QUOTE
						+ AttributeManager.PAIRS_SEPARATOR + AttributeManager.SINGLE_QUOTE + password
						+ AttributeManager.SINGLE_QUOTE + AttributeManager.PAIRS_SEPARATOR
						+ AttributeManager.SINGLE_QUOTE + AttributeManager.NEW_USER_ROLE
						+ AttributeManager.SINGLE_QUOTE + AttributeManager.CLOSE_PAREN);
			} else {
				JOptionPane.showMessageDialog(gui, "Username already exists", "Error", JOptionPane.ERROR_MESSAGE);
			}
			if (connection != null)
				connection.close();
		} catch (SQLException e) {
	 ...
	 * 
	 * @param dbName
	 *            is the database path and name
	 * @param status
	 *            is the enum type containing status of the message
	 * @param type
	 *            is the enum type containing type of the message return the
	 *            array containing messages with the given status and type
	 */
	public String[] getMessageByStatusAndType(String dbName, AttributeManager.MessageStatus status,
			AttributeManager.MessageType type) {
		...
		try {
			...
			if (status == AttributeManager.MessageStatus.ALL && type == AttributeManager.MessageType.ALL)
				rs = statement.executeQuery(AttributeManager.SELECT_FROM + DB_MESSAGE_TABLE
						+ AttributeManager.LINE_ENDER);
			else if (status == AttributeManager.MessageStatus.ALL && type != AttributeManager.MessageType.ALL)
				rs = statement.executeQuery(AttributeManager.SELECT_FROM + DB_MESSAGE_TABLE + AttributeManager.WHERE
						+ DB_MESSAGE_TYPE + AttributeManager.ATTR_VAL_SEPARATOR + type.getCode()
						+ AttributeManager.LINE_ENDER);
			else if (status != AttributeManager.MessageStatus.ALL && type == AttributeManager.MessageType.ALL)
				rs = statement.executeQuery(AttributeManager.SELECT_FROM + DB_MESSAGE_TABLE + AttributeManager.WHERE
						+ DB_MESSAGE_STATUS + AttributeManager.ATTR_VAL_SEPARATOR + status.getCode()
						+ AttributeManager.LINE_ENDER);
			else
				rs = statement.executeQuery(AttributeManager.SELECT_FROM + DB_MESSAGE_TABLE + AttributeManager.WHERE
						+ DB_MESSAGE_TYPE + AttributeManager.ATTR_VAL_SEPARATOR + type.getCode()
						+ AttributeManager.CONJUNCTION + DB_MESSAGE_STATUS + AttributeManager.ATTR_VAL_SEPARATOR
						+ status.getCode() + AttributeManager.LINE_ENDER);
			rs.next();
			while (!rs.isAfterLast()) {
				String msg = DB_MESSAGE_ID + AttributeManager.ATTR_VAL_SEPARATOR + rs.getString(DB_MESSAGE_ID)
						+ AttributeManager.PAIRS_SEPARATOR + DB_MESSAGE_SENDER + AttributeManager.ATTR_VAL_SEPARATOR
	...
	public void updateUpdate(Update u, String dbName) {
		...
		try {
			...
			statement.execute(AttributeManager.UPDATE + DB_MESSAGE_TABLE + AttributeManager.SET + DB_MESSAGE_STATUS
					+ AttributeManager.ATTR_VAL_SEPARATOR + AttributeManager.SINGLE_QUOTE + u.getStatus().getCode()
					+ AttributeManager.SINGLE_QUOTE + AttributeManager.WHERE + DB_MESSAGE_ID
					+ AttributeManager.ATTR_VAL_SEPARATOR + u.getId() + AttributeManager.LINE_ENDER);
	...
	public void insertMessage(String message, String sender, String dbName) {
		...
		try {
			...
			statement.execute(AttributeManager.INSERT + DB_MESSAGE_TABLE + AttributeManager.OPEN_PAREN
					+ DB_MESSAGE_COLUMN + AttributeManager.PAIRS_SEPARATOR + DB_MESSAGE_STATUS
					+ AttributeManager.PAIRS_SEPARATOR + DB_MESSAGE_SENDER + AttributeManager.CLOSE_PAREN
					+ AttributeManager.VALUES + AttributeManager.OPEN_PAREN + AttributeManager.SINGLE_QUOTE + message
					+ AttributeManager.SINGLE_QUOTE + AttributeManager.PAIRS_SEPARATOR + DB_STATUS_NEW
					+ AttributeManager.PAIRS_SEPARATOR + AttributeManager.SINGLE_QUOTE + sender
					+ AttributeManager.SINGLE_QUOTE + AttributeManager.CLOSE_PAREN);
	...
	public void readFile() {
		...
		log("Text read from " + filename + " : " + System.getProperty("line.AttributeManager.PAIRS_SEPARATOR")
				+ toString());
	}
	...
	// String[] commune =
	// {"Anse a PitresAttributeManger.PAIRS_SEPARATORBainetAttributeManger.PAIRS_SEPARATORBelle AnseAttributeManger.PAIRS_SEPARATORCote de ferAttributeManger.PAIRS_SEPARATORGrand GosierAttributeManger.PAIRS_SEPARATORLa valleeAttributeManger.PAIRS_SEPARATORThiotte"};
	// return commune;
	//
	// }
	//
	...
	public static void main(String args[]) throws Exception {
		...
		// }
		/*
		 * SmsReader reader = new SmsReader(); String filepath =
		 * System.getProperty(AttributeManager.USER_DIRECTORY).toString()+
		 * AttributeManager.DATABASE_PATHNAME + args[0];
		 * System.out.println("Path = " + filepath); for (MessageStatus st :
		 * MessageStatus.values()) { System.out.println(st); String[] test =
		 * reader.getMessageByStatus(filepath, st); for (MessageStatus st :
		 * MessageStatus.values()) { for (MessageType tp : MessageType.values())
		 * { System.out.println(st+ AttributeManager.CONJUNCTION +tp); String[]
		 * test =
		 * reader.getMessageByStatusAndType(System.getProperty(AttributeManager
		 * .USER_DIRECTORY).toString()+ "/db/haiti.db", st, tp); //String[] arr
		 * = reader.convertToArray(test); for (int i = 0; i < test.length; i ++)
		 * { System.out.println(test[i]); } }
		 * 
		 * for (MessageType st : MessageType.values()) { System.out.println(st);
		 * String[] test = reader.getMessageByType(filepath, st); //String[] arr
		 * = reader.convertToArray(test); for (int i = 0; i < test.length; i ++)
		 * { System.out.println(test[i]); }
		 */
		...
		String filepath = System.getProperty(AttributeManager.USER_DIRECTORY).toString()
				+ AttributeManager.DATABASE_PATHNAME + args[0];
		...
		/*
		 * if (args.length < 1) { reader = new SmsReader();
		 * reader.readUnprocessedMsgsFromDb("jdbc:sqlite:sample.db"); String[]
		 * arr = reader.getMessagesAsArray(); for (int i = 0; i < arr.length; i
		 * ++) { System.out.println(arr[i]); } } else if (args.length < 2) {
		 * reader = new SmsReader(args[0]); // Uses default encoding
		 * reader.readFile(); } else { reader = new SmsReader(args[0], args[1]);
		 * reader.readFile(); }
		 */
	}
}
