...
/**
 * This class manages all attributes and abbreviations on both the client
 * (mobile) and server side.
 * 
 * On the mobile side, it defines the mappings between Db column names, such as
 * 'firstname' and he abbreviation used for that attribute in the SMS messasge
 * ('f').
 * 
 * On the server side it defines the reverse mapping, from 'f' to 'firstname'.
 * 
 */
public class AttributeManager {
	...
	public static final String FORM_AGRICULTURE_2 = "Agr2"; // Agriculture
															// Program of
															// ACDI/VOCA?:";
	...
	public static final String[] isAFields = { "fa", "fi", "mu", "ra", "st",
			"ot", "at", "fo", "sv", "cr", "pl", "md", "pt", "ag", "ar", "bn",
			"rb" };
	...
	public static void init() {
		...
		abbreviations.put(ABBREV_MSG_NUMBER_SLASH_SIZE, LONG_MESSAGE_NUMBER);
		...
		// abbreviations.put(ABBREV_GENERAL_INFORMATION,
		// LONG_GENERAL_INFORMATION);
		...
		abbreviations.put(ABBREV_STATUS, LONG_MESSAGE_TYPE);

		// // This group maps SMS abbreviations to full attribute names (for the
		// server)
		...
		// This group maps Db column names in the on-phone Db to SMS
		// abbreviations
		abbreviations.put(FINDS_DOSSIER, ABBREV_DOSSIER);
		abbreviations.put(FINDS_BENEFICIARY_TYPE, ABBREV_TYPE);
		abbreviations.put(FINDS_MESSAGE_TYPE, ABBREV_STATUS);
		...
		abbreviations.put(FINDS_Q_VISIT_MOTHER_LEADER,
				ABBREV_VISIT_MOTHERLEADER);
		...
		abbreviations.put("Centre de santé un", "h1");
		...
		abbreviations.put("Centre de santé cinq", "h5");

		abbreviations.put("h1", "Centre de santé un");
		abbreviations.put("h2", "Centre de santé deux");
		abbreviations.put("h3", "Centre de santé trois");
		abbreviations.put("h4", "Centre de santé quatre");
		abbreviations.put("h5", "Centre de santé cinq");
	}
	 ...
	 * 
	 * @param abbreviatedAttributes
	 *            TRUE if abbreviated, FALSE if not
	 * @param s
	 *            the String to be mapped to long
	 * @return the long form of the String
	 */
	// public static String mapToLong(Beneficiary.Abbreviated
	// abbreviatedAttributes, String s){
	...
	// * @return the long form of the String
	// */
	// public String mapToLong(SmsMessage.Abbreviated abbreviatedAttributes,
	// String s){
	...
	/**
	 * Mobile side method. Converts attribute-value pairs to abbreviated
	 * attribute-value pairs.
	 * 
	 * @param attr
	 *            a long attribute name, e.g. 'firstname'
	 * @param val
	 *            a data value, e.g. 'Joseph'
	...
	//
	/**
	 * Mobile side. Convert a value to an abbreviated value. This is mostly used
	 * for the names of health centers and distribution posts.
	 * 
	 * @param val
	 *            a String of the form "Name of Some Health Center"
	 ...
	 * @param newAttr
	 *            a String giving the name for the int attribute in the message
	 *            -- e.g., 'is' or 'hs'
	 * @return an SMS where an attr/val pair such as 'is=9' replaces
	 *         'fa=1,ra=1'.
	 */
	public static String encodeBinaryFields(String smsMsg, String[] attributes,
			String newAttr) {
		int sum = 0;
		String result = "";
		String pairs[] = smsMsg.split(PAIRS_SEPARATOR);

		// Split each attr/value pair in the Sms message into its attr and its
		// val
		...
		System.out.println(TAG + "Decoded sum = "
				+ decodeBinaryFieldsInt(sum, attributes));
	...
	/**
	 * This is the reverse mapping of encodeBinaryFields. It takes a decimal
	 * (base-10) number and converts it to binary where 1 bits correspond to the
	 * attributes in the attributes array. Those are returns as a String of
	 * attr=val pairs. For example the codedInt 9 would translate to 1001 or 2^0
	 * + 2^3 or 'fa=1,ra=1' using the attributes array ["fa", "fi", "mu", "ra",
	 * "st", "ot"]
	 * 
	 * @param codedInt
	 *            is a base-10 number
	 * @param attributes
	 *            is an array of attribute abbreviations
	...
	public static String decodeBinaryFieldsInt(int codedInt, String[] attributes) {
		...
		for (int k = len; k >= 0; k--) {
			...
			if (codedInt >= pow2) {
				...
				result += attributes[k] + ATTR_VAL_SEPARATOR + "1"
						+ PAIRS_SEPARATOR;
			}
		}
		System.out.println(TAG + "Result = " + result);
	 ...
	 * @param arr
	 * @param s
	 * @return returns the index of s in arr or -1 if it is not contained
	 *         therein
	 */
	...
	/**
	 * TODO: This method should thoroughly test this class. For example, print
	 * out all mappings of short to long.
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		AttributeManager am = AttributeManager.getInstance(); // new
																// AttributeManager();
